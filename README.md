...
1. Terminate any stray machines that are found. Note that the reason why we terminate machines here is that the autoscaler only works well with a stable state where there is a 100% correspondence between physical (cloud provider) and logical (Spacelift) nodes.
